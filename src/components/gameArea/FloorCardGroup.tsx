...
import { CARD_OVERLAP, CARD_H, SLOT_W } from './floorLayout';
...
export const FloorCardGroup = memo(function FloorCardGroup({
  ...
  // position = 슬롯 중심 → 왼쪽 상단 앵커로 변환
  // SLOT_W 기준이므로 카드 수가 변해도 이 값은 불변
  const leftEdge = position.x - SLOT_W / 2;
  const topEdge = position.y - CARD_H / 2;
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, x: leftEdge, y: topEdge, scale: 1 }}
